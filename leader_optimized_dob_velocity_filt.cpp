...
int main()
{
  try
  {
    ...
    const std::vector<bool> active_joints = config["global"]["active_joints"].as<std::vector<bool>>();
    ...
    std::array<double, kJointCount> estimated_velocity{};
    bool velocity_observer_initialized = false;

    const double g_velocity = readOptionalDouble(config, "global", "g_velocity", 2000.0);
            ...
            franka::Duration period) -> franka::Torques
    {
      ...
      if (!velocity_observer_initialized)
      {
        ...
        estimated_velocity.fill(0.0);
        velocity_observer_initialized = true;
      }
      else
      {
        for (std::size_t i = 0; i < kJointCount; ++i)
      ...
      if (publish_state_mutex.try_lock())
      {
        ...
        shared_leader_data.vel = estimated_velocity;
      ...
      for (std::size_t i = 0; i < kJointCount; ++i)
      {
        ...
        const double velocity_error = estimated_velocity[i] - cached_follower_data.vel[i];

        const double velocity_sum = estimated_velocity[i] + cached_follower_data.vel[i];
        ...
        const double omega = estimated_velocity[i];
...
#error "A teleoperation mode must be selected"
      ...
      if (recording.load(std::memory_order_relaxed))
      {
        ...
        sample.leader_vel = estimated_velocity;
